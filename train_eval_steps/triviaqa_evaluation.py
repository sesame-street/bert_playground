...
def normalize_answer(s):
    ...
    def handle_punc(text):
        exclude = set(string.punctuation + u"".join([u"‘", u"’", u"´", u"`"]))
        return ''.join(ch if ch not in exclude else u' ' for ch in text)
...
def evaluate_triviaqa(ground_truth, predicted_answers, qid_list=None, mute=False):
    ...
    for qid in qid_list:
        ...
        if em_for_this_question == 0 and not mute:
            print("em=0:", repr(prediction), repr(ground_truths))
...
def get_args():
    ...
    parser.add_argument('--out_file', help='Evaluation output file')
...
if __name__ == '__main__':
    ...
    with codecs.open(args.out_file, mode='wt', encoding='utf8') as fout:
