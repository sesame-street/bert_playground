...
def validate_flags_or_throw(bert_config):
    ...
    if not FLAGS.label_cleaning and FLAGS.local_obj_alpha < 1e-6:
        raise ValueError("When not label cleaning, local_obj_alpha = 1.0")
    ...
    def build_loss(self):
        ...
        if FLAGS.train_first_answer:
            ...
            if FLAGS.label_cleaning:
                ...
                # Only considers answer spans corresponding to the best answer
                # string for the whole document.
                masked_ans_span_probs = ans_span_probs * tf.cast(
                    best_answer_position_masks, tf.float32)

                best_pos = tf.argmax(tf.reshape(masked_ans_span_probs, [-1]))
                best_answer_position_masks = tf.reshape(
                    tf.sparse_to_dense(
                        [best_pos],
                        output_shape=[batch_size * self.max_num_answers],
                        sparse_values=tf.cast(1, tf.int64)
                    ),
                    [batch_size, self.max_num_answers]
                )
